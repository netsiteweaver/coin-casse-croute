...
jQuery(function(){
    ...
    $('.daterange').daterangepicker({
        ...
        let qs = base_url + "orders/listing?start_date="+start.format("YYYY-MM-DD")+"&end_date="+end.format("YYYY-MM-DD")+"&search_text="+search_text+"&stage="+stage;
    ...
    $('#apply-filter').on('click',function(){
        let search_text = $('#search_text').val();
        let rpp = $('#rpp').val();
        let qs = base_url + "orders/listing?start_date="+selectedStartDate+"&end_date="+selectedEndDate+"&search_text="+search_text+ "&display="+rpp;
...
async function refreshOrders() {
    const response = await fetch( base_url + "ajax/orders/refresh");
    const orders = await response.json();
    $('#orders_listing tbody tr').empty();
    $(orders.orders).each(function(i,row){
        var line = "<tr class='text-center'>";
        line += "<td>"+(i+1)+"</td>";
        line += "<td>"+row.order_date.substr(0,10)+"</td>"
        line += "<td>"+row.document_number+"</td>"
        line += "<td>"+( (row.client===null)?'WALK IN':row.client )+"</td>"
        line += "<td>"+(row.amount).toLocaleString("en")+"</td>"
        line += "<td>"+row.agent+"</td>"
        line += "<td>";
        line += '<a href="' + base_url + 'orders/receipt/' + row.uuid + '"><div class="btn btn-default"><i class="fa fa-file"></i> View</div></a>';
        line += '<a href="' + base_url + 'orders/receipt/' + row.uuid + '?print"><div class="btn bg-purple"><i class="fa fa-print"></i> Print</div></a>';
        line += "</td>"
        line += "</tr>";
        $('#orders_listing tbody').append(line);
    })
}
